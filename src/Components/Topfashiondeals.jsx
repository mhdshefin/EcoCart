            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-5 text-black bg-[#fff] pt-5 pb-5 px-5 text-xl sm:text-2xl rounded-lg'>
                {
                    Topfashiondeals.map((item, index) => {
                        return (
                            <Link to={`/collection/${item.section}/${item.category}/${item.subCategory}`} key={index}><div className='sm:min-w-[200px] w-[180px] h-[280px] md:w-[250px] lg:w-[280px] xl:w-[300px] md:h-[380px] flex flex-col items-center border border-gray-300 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-xl'>
