            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-5 text-black bg-[#fff] pt-5 pb-5 px-5 text-xl sm:text-2xl rounded-lg'>
                {
                    bestSellerProducts.map((item, index) => {
                        ...
                        return (
                            <Link key={index} to={`/collection/${item.section}/${item.category}/${item.subCategory}`}><div className='sm:min-w-[200px] w-[160px] h-[270px] md:w-[250px] lg:w-[250px] xl:w-[280px] relative md:h-[360px] flex flex-col border border-gray-300 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-xl'>
                                <div className=' z-50 left-0 top-[4%] absolute bg-gradient-to-r from-[#5f98ee] via-[#8fbdfd] to-[#5f98ee] animate-shimmer bg-[length:200%_100%] w-1/2 xl:w-[40%] h-6 sm:h-7 sm:text-lg text-xs text-white rounded-r-[3px] flex items-center justify-start pl-1 shadow-xl font-medium'>
                                    <p>Best Seller</p>
                                </div>
